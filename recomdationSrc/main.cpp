...
int hybirdHAndPNonLinaer(double lamada, SimpleNetwork &network = combainTrainAndLearningSet)
{
	...
	double powerSeconsItemDegree[itemNum];
	double powerSecondUserDegree[userNum];
	memset(powerItemDegree, 0, sizeof(powerItemDegree));
	memset(powerUserDegree, 0, sizeof(powerUserDegree));
	memset(powerSecondUserDegree, 0, sizeof(powerSecondUserDegree));
	memset(powerSeconsItemDegree, 0, sizeof(powerSeconsItemDegree));
	double hybirdDegree = 0;
	//emptyVector(&Hybirdscore);
	//itemRankForUser.clear();
	for (int i = 0; i<userNum; i++)
	{
		...
		if (tsize>0)
		{
			for (int j = 0; j < tsize; j++)
			{
				...
				if (powerItemDegree[tempItemId] == 0)
				{
					...
					//cout << "start" << "\tuser: " << i << "\titem: " << tempItemId << "\t" << powerItemDegree[tempItemId] << "\t" << network.item_user_relation[tempItemId].size() << "\t" << lamada << endl;
				}
				for (unsigned int k = 0; k < network.item_user_relation[tempItemId].size(); k++)
				 {
					int linkBackUser = network.item_user_relation[tempItemId][k];
			...
			for (int m = 0; m<userNum; m++)
			{
				if (tempUserScore[m]>0)
				{
					if (powerSecondUserDegree[m] == 0)
					{
						powerSecondUserDegree[m] = pow((double)network.user_item_relation[m].size(), lamada);
					}
					for (unsigned int l = 0; l < network.user_item_relation[m].size(); l++)
					{
						int linkBackItem = network.user_item_relation[m][l];
						if (powerSeconsItemDegree[linkBackItem] == 0)
						{
							powerSeconsItemDegree[linkBackItem] = pow((double)network.item_user_relation[linkBackItem].size(), (1 - lamada));
						}
						hybirdDegree = powerSeconsItemDegree[linkBackItem] * powerSecondUserDegree[m];
...
int Heter_hybrid(double lamada,double sita, SimpleNetwork &network = combainTrainAndLearningSet)
{
	funcName = __FUNCTION__;
	cout<<"this is Heter_hybrid and lamada is :	"<<lamada<<endl;
	if (checkRuning())
	{		
		return -1;
	}
...
int Heter_PD(double lamada, double sita, SimpleNetwork &network = combainTrainAndLearningSet)
{
	...
	//emptyVector(&Hybirdscore);
	memset(powerItemDegree,0,sizeof(powerItemDegree));
	memset(powerInitScore, 0, sizeof(powerInitScore));
	//itemRankForUser.clear();
	for (int i = 0; i<userNum; i++)
	{
		memset(tempUserScore,0,sizeof(tempUserScore));
		...
		if (tsize>0)
		{
			...
			for (int j = 0; j < tsize; j++)
			{
				tempItemId = network.user_item_relation[i][j];
				itemusersize = network.item_user_relation[tempItemId].size();
				if (powerInitScore[tempItemId] == 0)
				{
					powerInitScore[tempItemId] = pow(itemusersize, lamada);
				}
				initialScore = powerInitScore[tempItemId];
			...
			for (int m = 0; m<userNum; m++)
			{
				if (tempUserScore[m]>0)
				{
					...
					for (unsigned int l = 0; l < network.user_item_relation[m].size(); l++)
					{
						...
						hybirdDegree = powerSecondUserDegree[m] * powerSeconsItemDegree[linkBackItem];
						//cout<<linkBackUser<<" "<<l<<" "<<linkBackItem<<endl;
...
void testifright()
{
	...
	for (double lamada = 0.0; lamada<=1.0; lamada+=0.02)
	{
		...
		cout<<"	RankingScore	"<<getRankingScore()<<endl;
